	...
	try {
		...
		try {
			...
			if (settings) {
				...
				if (!('updategempa' in settings)) settings.updategempa = true
			...
			if (datas) {
				...
				if (!('linkgc' in datas)) datas.linkgc = ''
				if (!('bingkey' in datas)) datas.bingkey = [{cookie: "Not Found", limit: "0"}]
				...
				if (!('fontstyle' in datas)) datas.fontstyle = '1'
				if (!('listultah' in datas)) datas.listultah = ''
			} else db.data.datas = {
				maingroupname: '',
				aiprofile: '',
				packname: '',
				...
				linkgc: '',
				wgempa: '',
				bingkey: [{cookie: "Not Found", limit: "0"}],
				spamcountreset: 0,
				spamcountgcreset: 0,
				spamlistmsg : null,
				spamlistgcmsg: null,
		...
		for (let name in plugins) {
			...
			if ((usedPrefix = (match[0] || '')[0])) {
				...
				if (m.chat in db.data.chats || m.sender in db.data.users) {
					let chat = db.data.chats[m.chat]
					let user = db.data.users[m.sender]
					let ubc = isLinux ? 'Owner-Menu/own-unbanchat.js' : 'Owner-Menu\\own-unbanchat.js'
					let ubu = isLinux ? 'Owner-Menu/unbanuser.js' : 'Owner-Menu\\unbanuser.js'
...
// let msg = await getContentType(message)
// if (!msg) return null
// return getMsg(msg?.message) || msg
// }
...
let msg = {
		rowner: `▷ ᴋʜᴜsᴜs ᴏᴡɴᴇʀ ◁`,
		owner: `▷ ᴋʜᴜsᴜs ᴏᴡɴᴇʀ ◁`,
		mods: `▷ ᴋʜᴜsᴜs ᴍᴏᴅᴇʀᴀᴛᴏʀ ◁`,
		premium: `▷ ᴋʜᴜsᴜs ᴜsᴇʀ ᴘʀᴇᴍɪᴜᴍ ◁\n_contact owner untuk upgrade ke premium_`,
		group: `▷ ᴋʜᴜsᴜs ᴅɪ ᴅᴀʟᴀᴍ ɢʀᴏᴜᴘ ◁`,
		private: `▷ ᴋʜᴜsᴜs ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ ◁`,
		admin: `▷ ᴋʜᴜsᴜs ᴀᴅᴍɪɴ ɢʀᴏᴜᴘ ◁`,
		nsfw: `▷ ɴғs ᴛɪᴅᴀᴋ ᴅɪ ᴀᴋᴛɪғᴋᴀɴ ◁`,
		game: '▷ ᴀᴋᴛɪғᴋᴀɴ ᴍᴏᴅᴇ ɢᴀᴍᴇ ◁',
		botAdmin: `▷ ʙᴏᴛ ᴛɪᴅᴀᴋ ᴀᴅᴍɪɴ ◁`,
		unreg: '▷ ᴀɴᴅᴀ ʙᴇʟᴜᴍ ᴛᴇʀᴅᴀғᴛᴀʀ ᴅɪ ᴅᴀᴛᴀʙᴀsᴇ ◁\n_#daftar nama.umur_',
		restrict: '▷ ғɪᴛᴜʀ ᴅɪ ᴅɪsᴀʙʟᴇ ◁'
	}[type]
